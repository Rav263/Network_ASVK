...
app.layout = html.Div([
    html.Div([
        ...
        html.Div(style={'width': '50%', 'display': 'inline'}, children=[
            'Strat vertex:',
            dcc.Input(id='input-vertex-start', type='text')
        ]),
        html.Div(style={'width': '50%', 'display': 'inline'}, children=[
            'End vertex:',
            dcc.Input(id='input-vertex-end', type='text')
        ]),
...
def send_data_to_server(edges, nodes, start, end):
    sock = socket.socket()
...
@app.callback(Output('info', 'children'),
            ...
            [State('cytoscape-elements', 'elements'),
             State('input-vertex-start', 'value'),
             State('input-vertex-end', 'value')])
def cacl_path(btn_calc, elements, start, end):
    global last_calc
    if int(btn_calc) == int(last_calc):
        return ""
    if start == None or end == None:
        return "please enter start and end nodes"

    sock = send_data_to_server(edges, nodes, start, end)
